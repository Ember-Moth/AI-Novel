...
- **Lint:** `bun run lint` — runs `oxlint --deny-warnings .` for static analysis.
...
- **Class names:** Use `cn` from `src/shared/cn.ts` for conditional or overridable Tailwind class composition. Do not inline `twMerge` in components unless there is a local one-off reason.
