...
function RowHoverSlot({ actions, badge }: { actions?: ReactNode; badge?: ReactNode }) {
  ...
  return (
    <div className="grid h-5 shrink-0 items-center">
      {actions ? (
        <div className="pointer-events-none col-start-1 row-start-1 flex w-0 items-center justify-end gap-1 justify-self-end overflow-hidden opacity-0 transition-all duration-200 interpolate-size group-hover:pointer-events-auto group-hover:w-auto group-hover:opacity-100">
          {actions}
        </div>
      ) : null}
      {badge ? (
